...
async function handleTransaction(
  ...
  if ((await triggersFrontRun(transaction, out_token_address)) === true) {
    ...
    var amounts = await uniswapRouter.methods
      ...
      .call();
    let output_amount = BigNumber(amounts[amounts.length - 1]).minus(
...
async function triggersFrontRun(transaction, out_token_address) {
  ...
  if (method == SEEFT) {
    ...
    if (out_token_addr != out_token_address) {
      ...
      console.log("out_token_addr != out_token_address : 1");
      return false;
    }

    console.log("");
    log_str =
      "target txn: swapExactETHForTokens : " +
      transaction["hash"] +
      " " +
      gasPrice.toFixed(2) +
      " GWEI " +
    ...
    if (BigNumber(in_amount).gte(BigNumber(pool_info.attack_volumn))) {
      console.log("My log : in_amount is more");
      ...
      console.log("My log : in_amount is less");
      return false;
    }
    ...
    if (out_token_addr != out_token_address) {
      ...
      console.log("in_token_addr :" + in_token_addr);
      console.log("out_token_addr :" + out_token_addr);
      console.log("out_token_addr != out_token_address : 2");
      return false;
    }

    if (BigNumber(in_max).gte(BigNumber(pool_info.attack_volumn))) {
      console.log("My log : in_amount is more 2");
      log_str =
        "target txn: swapETHForExactTokens " +
        transaction["hash"] +
        " " +
        gasPrice.toFixed(2) +
        " GWEI " +
      ...
      console.log("My log : in_amount is less 2");
      return false;
    }
  }
...
async function swap(
  ...
  try {
    ...
    if (trade == 0) {
      //buy
      swap =
        uniswapRouter.methods.swapExactETHForTokensSupportingFeeOnTransferTokens(
          outputtoken,
      ...
      //sell
      swap =
        uniswapRouter.methods.swapExactTokensForETHSupportingFeeOnTransferTokens(
          outputtoken,
          outputeth,
      ...
      .on("transactionHash", function (hash) {
        if (trade == 0) {
          let log_str = `buy swap : ${hash} ${
            outputeth / 10 ** 18
          } ETH ${Web3.utils.fromWei(gasPrice.toString(), "gwei")} GWEI `;
          logger.info(log_str + " " + getDateTime());
          log.info(log_str);
          console.log(log_str);
        } else {
      ...
      .on("receipt", async function (receipt) {
        if (receipt["status"]) {
          ...
          if (trade == 0) {
            ...
            let log_str = `sell success! swaped ETH: ${
...
async function getCurrentGasPrices() {
  ...
  // return prices;
  var response = await axios.get("https://owlracle.info/eth/gas");
...
async function preparedAttack(
  ...
  var OUT_TOKEN_ABI_REQ =
    "https://api.etherscan.io/api?module=contract&action=getabi&address=" +
    out_token_address +
    "&apikey=5UP3S1V4A7M9C4RSIJTH9YSANQMK99MNHU";
...
console.log(colours.fg.red, "INSUFFICIENT_BALANCE!");
log_str = "Your wallet balance must be more " + "4ETH " + "0.05 Gas ";
